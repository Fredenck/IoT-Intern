...
# Changelog:
# 2012 / 01 / 10 - Speed enhancement by rewriting diag() functions # with linear indices.

def veins (img, fvr, sigma):
    # Construct filter kernels
    winsize = math.ceil(4 * sigma)
    [X, Y] = cv2.meshgrid(-winsize:winsize, -winsize: winsize);
    ...
    fx = cv2.imfilter(img, hx, 'replicate', 'conv');
    ...
    while ~done
        ...
        if (Wr > 0 & & (y == img_h | | x == img_w | | ~bla(y, x)))
            ...
            than
            diag(..)
            d = k(((pos_x_start - 1) * img_h + pos_y_start + 2 * img_w * img_h):(img_h + 1): (
                        (pos_x_end - 1) * img_h + pos_y_end + 2 * img_w * img_h));
            [~, I] = max(d);
    ...
    while ~done
        ...
        if (Wr > 0 & & (y == 1 | | x == img_w | | ~bla(y, x)))
            ...
            than
            diag(flipud(..))
            d = k(((pos_x_start - 1) * img_h + pos_y_start + 3 * img_w * img_h):(img_h - 1): (
                        (pos_x_end - 1) * img_h + pos_y_end + 3 * img_w * img_h));
            [~, I] = max(d);
            pos_x_max = pos_x_start + I - 1;
            pos_y_max = pos_y_start - I + 1;
