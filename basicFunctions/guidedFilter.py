"""
From https://github.com/swehrwein/python-guided-filter/blob/master/gf.py
"""
import cv2
import numpy as np
import scipy as sp
...
import imageio
im = imageio.imread("../usefulImages/modelVein.jpg").astype(np.float32) / 255
'''
gf = guided_filter(im, im, r, eps)
gfs4 = guided_filter(im, im, r, eps, s=4)
cv2.imshow("gf", gf)
cv2.waitKey(0)
cv2.imshow("gfs4", gfs4)
cv2.waitKey(0)
...
tulips_smoothed = np.zeros_like(im)
for i in range(3):
    tulips_smoothed[:,:,i] = guided_filter(im, im[:,:,i], r, eps)
cv2.imshow("a", tulips_smoothed)
cv2.waitKey(0)
